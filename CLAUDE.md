...
- **Two-stage fire-and-forget session logging.** Logging is split across the two endpoints so the Drive upload happens at analyse-time (when the image is in hand) rather than being plumbed through `/match`. Each stage runs in its own `daemon=True` thread and returns immediately; the user response is never blocked, the worker can shut down without waiting, and all exceptions inside the `_do_log_*` workers are caught via `log.exception(...)` — nothing escapes.
  - **Stage 1 — `/analyse` → `log_analyse_session(...)`**: `/analyse` mints a `session_id = uuid.uuid4()` at the top of the handler, runs `analyse_outfit`, and (on success) calls `log_analyse_session` with the image, occasion, outfit JSON, analyse model, analyse time, and confidence flag. The thread uploads the image to `DRIVE_FOLDER_ID` as `{session_id}.jpg` with `anyone/reader` permission, ensures the `Sessions` header row exists, and appends an 8-element row populating columns **A–H** (Session ID, Timestamp UTC, Occasion, Analyse Model, Analyse Time, Confidence Flag, Outfit Analysis JSON, Drive Image URL). Columns I–N are intentionally left blank. The response body now includes `session_id` so the frontend can pass it to `/match`.
...
- **Logger transport: Drive image + 14-column Sheets row, split across two writes.** Column layout is analyse-side first (A–H), match-side after (I–N) — see `HEADER_ROW` in [logger.py](logger.py) for the canonical order. Auth via a service account; the full JSON lives in env var `GOOGLE_SERVICE_ACCOUNT_JSON` (one-line string) and is parsed with `json.loads` at log time. Scopes: `drive.file` + `spreadsheets`. The target Drive folder and target Sheet must both be shared with the service account email as Editor.
...
- **`app.py`** — Flask REST API. `POST /analyse` (sync, ~6s blocking) returns `{ success, session_id, outfit_analysis }` and `GET /health` is a normal blocking endpoint. `POST /match` is asynchronous: it accepts an optional `session_id`, queues a job, and returns a `job_id`. `GET /result/<job_id>` polls the job state and returns `running` / `done` / `error`. Done and errored jobs are deleted on fetch. CORS enabled for all origins. `preload_catalog()` runs at module-import time so the first request isn't slow.
- **`logger.py`** — two-stage fire-and-forget session logger. Public surface is two functions: `log_analyse_session(...)` (Drive upload + new row populating A–H) and `log_match_session(...)` (in-place update of I–N keyed on `session_id`). Each spawns its own `daemon=True` thread and returns immediately. Internal `_do_log_analyse` / `_do_log_match` workers wrap their work in `try/except → log.exception(...)`; nothing escapes. The 14-column `HEADER_ROW` constant is the canonical column order.
