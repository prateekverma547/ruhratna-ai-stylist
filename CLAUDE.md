...
- **`app.py`** — Flask REST API. `POST /analyse` (sync, ~6s blocking) returns `{ success, session_id, outfit_analysis }` on a usable image, or the rejection shape `{ success: false, error: true, confidence_flag: 0, rejection_reason, session_id, outfit_analysis: null }` when Gemini flags the image — **both at HTTP 200**. `GET /health` is a normal blocking endpoint. `POST /match` is asynchronous: it accepts an optional `session_id`, queues a job, and returns a `job_id`. `GET /result/<job_id>` polls the job state and returns `running` / `done` / `error`. Done and errored jobs are deleted on fetch. CORS enabled for all origins. `preload_catalog()` runs at module-import time so the first request isn't slow.
...
| `Procfile` | `gunicorn app:app --worker-class gevent --workers 2 --worker-connections 100 --timeout 120 --log-level info --capture-output --enable-stdio-inheritance --bind 0.0.0.0:$PORT` |
